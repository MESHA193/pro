...
function getSubserviceData(serviceId, featureName) {
    ...
    const specificData = {
        ...
        'Мероприятия по сокращению выбросов (НМУ)': {
            description: 'Мероприятия по уменьшению выбросов загрязняющих веществ в атмосферный воздух в периоды неблагоприятных метеорологических условий (НМУ) разрабатываются и реализуются юридическими лицами и индивидуальными предпринимателями, имеющими источники выбросов загрязняющих веществ в атмосферный воздух. Разработка данной документации позволяет хозяйствующим субъектам провести оценку влияния выбросов предприятия на окружающую среду, выявить перечень источников, на которых необходимо будет снижать выбросы в период наступления НМУ. Для разработки мероприятий по сокращению выбросов (НМУ) необходима в обязательном порядке актуальный проект по инвентаризации выбросов.',
            price: 'от 50 000 ₽',
            duration: '2-3 недели',
        ...
        'Проект нормативов образования отходов (ПНООЛР)': {
            description: 'Проект нормативов образования отходов (ПНООЛР) - документ, включающий в себя сведения обо всех отходах предприятия, их объемах, свойствах, местах накопления, заключенных договорах.',
            price: 'от 45 000 ₽',
            duration: '2-3 недели',
...
function openSubserviceOrderModal(serviceId, featureIndex) {
    ...
    // Закрываем список услуг и открываем модальное окно подуслуги
    ...
    if (subserviceData.whoNeeds && whoNeedsSection) {
        ...
        whoNeedsSection.querySelector('.info-text').innerHTML = subserviceData.whoNeeds;
    }
    ...
    if (subserviceData.necessaryFor && necessaryForSection) {
        ...
        necessaryForSection.querySelector('.info-text').innerHTML = subserviceData.necessaryFor;
    }

    const additionalInfoSection = document.getElementById('subserviceModalAdditionalInfo');
    if ((subserviceData.additionalInfo || subserviceData.stages) && additionalInfoSection) {
        additionalInfoSection.style.display = 'block';
        additionalInfoSection.querySelector('.info-text').innerHTML = (subserviceData.additionalInfo || subserviceData.stages);
    }

    // Меняем заголовок особенностей
    ...
    if (modalFeaturesBlock) {
        ...
        if (featuresTitle) {
            featuresTitle.textContent = (featureName === 'Экологическое сопровождение предприятий')
    ...
    const modalFeaturesList = document.getElementById('subserviceModalFeaturesList');
    ...
    if (featureName === 'Экологическое сопровождение предприятий') {
        if (!komuSection) {
            ...
            if (desc && desc.parentNode) desc.parentNode.insertBefore(komuSection, desc.nextSibling);
        } else {
    ...
    if (featureName === 'Экологический аудит') {
        if (!auditSection) {
            ...
            auditSection.innerHTML = '<h4 style="margin-bottom:0.4em;">Этапы:</h4>' +
                ...
                '</ul>';
            const desc = document.getElementById('subserviceModalDescription');
            if (desc && desc.parentNode) desc.parentNode.insertBefore(auditSection, desc.nextSibling);
        } else {
    ...
    if (featureName === 'Паспортизация отходов') {
        if (!passportSection) {
            ...
            passportSection.innerHTML = '<h4 style="margin-bottom:0.4em;">Вы получаете:</h4>' +
                ...
                '</ul>';
            const desc = document.getElementById('subserviceModalDescription');
            if (desc && desc.parentNode) desc.parentNode.insertBefore(passportSection, desc.nextSibling);
    ...
    // Скрываем блок особенностей для "Паспортизация отходов"
    if (modalFeaturesBlock) {
        if (featureName === 'Паспортизация отходов') {
    ...
    const externalControls = modal.querySelector('.modal-external-controls');
    if (externalControls) externalControls.style.display = 'flex';

    const backBtn = document.getElementById('subserviceBackToListBtn');
