...
function getSubserviceData(serviceId, featureName) {
    ...
    const specificData = {
        'Инвентаризация стационарных источников выбросов': {
            ...
            validity: 'Для объектов НВОС I категории – 7 лет в составе Комплексного экологического разрешения (КЭР)\nДля объектов НВОС II категории – 7 лет в составе Декларации о воздействии на окружающую среду (ДВОС)\nДля объектов НВОС III и IV категории – до изменений в технологическом производстве, а также случаев изменения в количественном и качественном составе выбросов атмосферу.',
            necessaryFor: 'Инвентаризация стационарных источников выбросов необходима для:\n- постановка на учёт объектов НВОС\n- разработка проекта нормативов допустимых выбросов (НДВ/ПДВ)\n- разработка плана мероприятий по сокращению выбросов (НМУ)\n- разработка проекта санитарной защитной зоны (СЗЗ)\n- разработка программы ПЭК\n- декларация о воздействии на окружающую среду (ДВОС)\n- комплексного экологического разрешения (КЭР)',
            workStages: 'Этапы работ: сбор исходной информации – обработка, проведение расчётов - оформление документации.',
...
function openSubserviceOrderModal(serviceId, featureIndex) {
    ...
    // Сначала скрываем все дополнительные секции
    const allInfoSections = modal.querySelectorAll('.info-section');
    ...
    if (modalDescription) modalDescription.innerHTML = subserviceData.description.replace(/\n/g, '<br>');
    if (modalPrice) {
    ...
    // Дополнительные секции - показываем только если есть данные
    if (subserviceData.whoNeeds) {
        const whoNeedsSection = document.getElementById('subserviceModalWhoNeeds');
        if (whoNeedsSection) {
            whoNeedsSection.style.display = 'block';
            const whoNeedsText = whoNeedsSection.querySelector('.info-text');
            if (whoNeedsText) {
                whoNeedsText.innerHTML = subserviceData.whoNeeds.replace(/\n/g, '<br>');
            }
        }
    }

    if (subserviceData.validity) {
        ...
        if (validitySection) {
            ...
            if (validityText) validityText.innerHTML = subserviceData.validity.replace(/\n/g, '<br>');
        }
    }

    if (subserviceData.necessaryFor) {
        const necessaryForSection = document.getElementById('subserviceModalNecessaryFor');
        if (necessaryForSection) {
            necessaryForSection.style.display = 'block';
            const necessaryForText = necessaryForSection.querySelector('.info-text');
            if (necessaryForText) necessaryForText.innerHTML = subserviceData.necessaryFor.replace(/\n/g, '<br>');
        }
    }

    if (subserviceData.approval) {
        ...
        if (approvalSection) {
            ...
            if (approvalText) approvalText.innerHTML = subserviceData.approval.replace(/\n/g, '<br>');
        }
    }

    if (subserviceData.workStages) {
        ...
        if (additionalInfoSection) {
            ...
            if (additionalInfoText) additionalInfoText.innerHTML = subserviceData.workStages.replace(/\n/g, '<br>');
        }
    }
